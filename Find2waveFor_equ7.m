...
for i=1:1:(fL-2)
    ...
    for i = 1:1:size(datas,1)
        ...
        for j = i:1:size(datas,1)
                ...
                Fit_in_vector = 1 ./ datas(i,:) - 1 ./ datas(j,:);
    ...
    %--------------------------------------------------------------------
    savename = strcat('D:\yunxia\ML\huigui 老公\result_equ7\','R_matrix',nm_vd_output);
    savename = savename(1:end-5);
    save(savename,'R_matrix');
    savename = strcat('D:\yunxia\ML\huigui 老公\result_equ7\','R_max',nm_vd_output);
    savename = savename(1:end-5);
    save(savename,'R_max');
    savename = strcat('D:\yunxia\ML\huigui 老公\result_equ7\','wave_selected',nm_vd_output);
    savename = savename(1:end-5);
