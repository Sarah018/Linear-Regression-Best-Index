...
for i=5:1:(fL-2)
    ...
    for i = 1:1:size(datas,1)
        ...
        for j = i:1:size(datas,1)
            for k = j:1:size(datas,1)
                %Fit_in_vector = (datas(i,:) - datas(j,:))./(datas(i,:) - datas(k,:));%computing according ()
               % Fit_in_vector = datas(i,:)./datas(j,:);
              %  Fit_in_vector = (datas(i,:) - datas(j,:))./(datas(i,:) - datas(k,:));
                Fit_in_vector = (datas(i,:))./(datas(j,:).* datas(k,:));
    ...
    save('R_matrix_equ4','R_matrix');
    save('R_max_equ4','R_max');
    save('wave_selected_equ4','wave_selected');
    %上面求出的是一个上三角矩阵，下面把它对称的部分补齐
%     for i = 1:size(R_matrix,1)
%         for j = 1:i
%             for k = 1:j
%                 R_matrix(i,j,:) = R_matrix(j,i,:);%这句有问题！！！需要改，想想怎么改
%             end
%         end
%     end
    R_matrix_new = zeros(size(datas,1),size(datas,1),size(datas,1));
    for i = 1:size(R_matrix_new,1)
        for j = i:size(R_matrix_new,1)
            for k = j:size(R_matrix_new,1)
                ijk = sort([i j k]);
                ii = ijk(1);
                jj = ijk(2);
                kk = ijk(3);
                R_matrix_new(i,j,k) = R_matrix(ii,jj,kk);
                R_matrix_new(i,k,j) = R_matrix(ii,jj,kk);
                R_matrix_new(j,i,k) = R_matrix(ii,jj,kk);
                R_matrix_new(j,k,i) = R_matrix(ii,jj,kk);
                R_matrix_new(k,j,i) = R_matrix(ii,jj,kk);
                R_matrix_new(k,i,j) = R_matrix(ii,jj,kk);
            end
        end
    end
    ...
    %--------------------------------------------------------------------
    savename = strcat('D:\yunxia\ML\huigui 老公\resultR1proR2\','R_matrix',nm_vd_output);
    savename = savename(1:end-5);
    save(savename,'R_matrix');
    savename = strcat('D:\yunxia\ML\huigui 老公\resultR1proR2\','R_max',nm_vd_output);
    savename = savename(1:end-5);
    save(savename,'R_max');
    savename = strcat('D:\yunxia\ML\huigui 老公\resultR1proR2\','wave_selected',nm_vd_output);
    savename = savename(1:end-5);
