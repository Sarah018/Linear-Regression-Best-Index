...
%Pt_Subs = 'D:\yunxia\ML\回归\resultR1toR2';%存放待处理数据的路径
Pt_Subs = 'D:\yunxia\ML\huigui\resultR1proR2';%存放待处理数据的路径
Nm_Vd = [];%存放文件名的结构体
...
for i=1:1:5
    nm_vd = Nm_Vd{i,1};
    %Local = strcat('D:\yunxia\ML\回归\resultR1toR2\',nm_vd);
    ...
    for j = 1:3
        ...
        nm_vd = nm_vd(1:end-4);
        %savename = strcat('D:\yunxia\ML\回归\figureR1toR2\',nm_vd,'.jpeg');
        ...
        savename2 = strcat('D:\yunxia\ML\huigui\resultExcel_R1proR2\',nm_vd,num2str(j),'.xlsx');%存为xls文件
        %savename(end-3:end) = [];
        %save(savename,'datas');
...
for i=6:1:10
    ...
    datas = datas.R_max;
    nm_vd = nm_vd(1:end-4);
    savename = strcat('D:\yunxia\ML\huigui\resultExcel_R1proR2\',nm_vd,'.xlsx');%存为xls文件
    %savename(end-3:end) = [];
    %save(savename,'datas');
